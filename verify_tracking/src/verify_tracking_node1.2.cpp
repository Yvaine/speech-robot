...
int main(int argc, char *argv[]) {
	...
	ros::Publisher verify_pub = n.advertise<std_msgs::Char>("verify_chatter", 1000);

	Subscriber track_pub = n.subscribe("track_chatter", 3, trackCallback);
			...
			if ( feature_length_2 > 0) {
				...
				if (cv_result == CV_OK) {
					...
					if (max_score > DEFAULT_THRESHOLD)
					{
						...
						spinOnce();
						printf("track node recv track flag %c\n", track_flag);
						if(track_flag == '0')
							continue;
