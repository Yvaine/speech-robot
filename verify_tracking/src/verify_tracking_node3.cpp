...
int face_track(VideoCapture &capture, Mat &trackframe, Point &expect, int frame_width,  int frame_height, cv_handle_t &handle_track, ros::Publisher motor_pub);
	...
	while(capture.isOpened()) {
		...
		if (face_count_2 > 0) {
			...
			if ( feature_length_2 > 0) {
				...
				// compare feature
				printf("begin to compare feature from camera with db\n");
				for(int i = 0; i < person_number; i++)
				{
				...
				if (cv_result == CV_OK) {
					...
					if (max_score > DEFAULT_THRESHOLD)
					{
						...
						capStatus = Verified;
						int track_value = face_track(capture, track_frame, expect, frame_width, frame_height, handle_track, motor_pub);
						if(track_value == -1)
