/*************************************************************************
	> File Name: verify_tracking_node.cpp
	> Author: lengjing
	> Mail: jingleng77@163.com 
	> Created Time: Mon Nov  9 15:04:40 2015
 ************************************************************************/
...
int face_track(VideoCapture &capture, Mat &trackframe, Point &expect, int frame_width,  int frame_height, cv_handle_t &handle_track, ros::Publisher servox_pub, ros::Publisher servoy_pub ,ros::Publisher motor_pub);
			...
			if ( feature_length_2 > 0) {
				...
				if (cv_result == CV_OK) {
					...
					if (max_score > DEFAULT_THRESHOLD)
					{
						...
						capStatus = Verified;
						int track_value = face_track(capture, track_frame, expect, frame_width, frame_height, handle_track, servox_pub, servoy_pub,motor_pub);
						if(track_value == -1)
						{
...
int face_track(VideoCapture &capture, Mat &trackframe, Point &expect, int frame_width,  int frame_height, cv_handle_t &handle_track,ros::Publisher servox_pub, ros::Publisher servoy_pub, ros::Publisher motor_pub)
{
	...
	int face_count = 0;
	cv_result_t cv_result = CV_OK;
    while(capStatus == Verified && capture.read(trackframe))
	{
		Mat temp_frame, color_frame;
		trackframe.copyTo(temp_frame);
