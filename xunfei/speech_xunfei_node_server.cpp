...
char *speech_cmd[CMD_NUM] = {"左转", "右转", "前进", "后退", "停止", "头抬起", "头低下", "头左转", "头右转"};
char cmd_code[CMD_NUM] = {'a', 'd', 'w', 's', 'q',' u', 'j', 'h', 'k'};
			...
			if (MSP_REC_STATUS_SUCCESS == rec_stat) //已经有部分听写结果
			{
				...
				if (NULL != rslt)
				{
					...
					for (int i = 0; i < CMD_NUM; i++)
					{
						...
						{
							printf(" speech_cmd is %s cmd_code is %c", speech_cmd[i], cmd_code[i]);
