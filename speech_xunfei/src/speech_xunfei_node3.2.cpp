 ...
 * Add host verify publisher to verify_tracking node  2015.11.25
 */
...
void convert(char speech_cmd, Publisher motor_pub, Publisher servox_pub, Publisher servoy_pub, Publisher track_pub, Publisher host_pub)
{		
	switch(speech_cmd)
	{
		...
		case 't':
			host_flag = '1';
			host.data = host_flag;
			host_pub.publish(host);
			printf("host verify flag %c\n", host_flag);
			fflush(stdout);
			break;
		default :
			motor.data = 'q';
...
void *recv_cmd_func(void *args)
{
	...
	Publisher host_pub = n.advertise<std_msgs::Char>("host_chatter", 1);
	...
	while(1)
	{
		...
		if((recvlen = recv(client_sockfd, &speech_cmd, 1, 0)) == 1)
		{
			...
			//motor.data = speech_cmd;
			//motor_pub.publish(motor);
			convert(speech_cmd, motor_pub, servox_pub, servoy_pub, track_pub, host_pub);
#if 0
